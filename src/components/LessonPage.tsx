...
import { ArrowLeft, BookOpen, Code, Target, Eye } from "lucide-react";
...
// Helper function to map lesson ID to visualization type
function getVisualizationType(lessonId: number): 'variables' | 'functions' | 'arrays' | 'objects' | 'interfaces' | 'classes' {
  const typeMap = {
    0: 'variables' as const,
    1: 'functions' as const,
    2: 'arrays' as const,
    3: 'objects' as const,
    4: 'interfaces' as const,
    5: 'classes' as const,
  };
  return typeMap[lessonId as keyof typeof typeMap] || 'variables';
}
      ...
      >
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid w-full grid-cols-2 bg-black/40 border-purple-500/30">
            ...
            <TabsTrigger
              value="visualization"
              className="flex items-center gap-2 data-[state=active]:bg-green-600/50 data-[state=active]:text-white"
            >
              <Eye size={16} />
              Visualize
            </TabsTrigger>
          ...
          <TabsContent value="visualization" className="space-y-6">
            <InteractiveVisualization
              type={getVisualizationType(lesson.id)}
              onInteraction={(data) => {
                console.log('User interaction:', data);
                // You can add more interactive logic here
              }}
            />
          </TabsContent>
